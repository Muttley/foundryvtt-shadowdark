...
export default class LightSourceTrackerSDv2 extends HandlebarsApplicationMixin(ApplicationV2) {
	...
	static DEFAULT_OPTIONS = {
		...
		actions: {
			douse: LightSourceTrackerSDv2.#onDouse,
			expand: LightSourceTrackerSDv2.#onExpand,
			lightsOut: LightSourceTrackerSDv2.#onLightsOut,
			openCharacterSheet: LightSourceTrackerSDv2.#onOpenCharacterSheet,
		},
	};
	...
	static PARTS = {
		lightSources: {
			root: true,
			template: templatePath("apps/v2/light-tracker"),
		},
	};

	static #onDouse(event, target) {
		this._onDouse(event, target);
	}
	...
	static #onLightsOut(event, target) {
		this._lightsOut();
	}

	static #onOpenCharacterSheet(event, target) {
	...
	async _onFirstRender(context, options) {
		...
		const uiLeftColumn = document.querySelector("#ui-left-column-1");
		if (!uiLeftColumn) {
			shadowdark.error("Error: Could not find #ui-left-column-1!");
			return;
		}

		const players = uiLeftColumn.querySelector("#players");
		if (!players) {
			shadowdark.warn(
				"Could not find #players HTML element, appending Momentum Tracker to end of ui-left-column-1."
			);
			uiLeftColumn.appendChild(this.element);
			return;
		}

		uiLeftColumn.insertBefore(this.element, players);
	}
	...
	async _prepareContext(_options) {
		let activeLightSourceCount = 0;
		for (const actorData of this.monitoredLightSources) {
			activeLightSourceCount += actorData.lightSources.length;
			actorData.showOnTracker = this.showAllPlayerActors || actorData.lightSources.length > 0;
		}

		const context = {
			activeLightSourceCount,
	...
	toggleExpanded(expanded) {
		...
		// this.element.classList.toggle("expanded", this.showAllPlayerActors);
		this.render(true);
	}

	async toggleInterface() {
		return ui.notifications.warn(
			game.i18n.localize("SHADOWDARK.notify.shadowdark_light_tracker_macro_deprecated"),
			{permanent: true}
		);
	}
...
const BaseLightTrackerLayer = foundry.canvas.layers.InteractionLayer;

class LightTrackerLayer extends BaseLightTrackerLayer {
	static get layerOptions() {
		return foundry.utils.mergeObject(super.layerOptions ?? {}, {
			name: "LightSourceTrackerSDv2",
			zIndex: 0,
		});
	}
}

export function registerLightTrackerLayer() {
	CONFIG.Canvas.layers ??= {};
	if (!CONFIG.Canvas.layers.lightTracker) {
		CONFIG.Canvas.layers.lightTracker = {
			layerClass: LightTrackerLayer,
			group: "interface",
		};
	}
}
